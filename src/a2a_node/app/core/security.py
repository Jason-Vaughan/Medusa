...
async def verify_medusa_handshake(
    ...
    1. A cryptographic signature (X-Medusa-Signature)
    2. Timestamp validity to prevent replay attacks (X-Medusa-Timestamp)
    3. (Optional/Legacy) The shared secret (X-Medusa-Secret)
    
    Returns the x_medusa_client_id if provided, otherwise "unknown-client".
    """
    # 1. Verify Timestamp (max 60 seconds skew)
    ...
    if not verify_signature(x_medusa_signature, payload, settings.A2A_SECRET):
        # Even if signature fails, we don't fallback to secret for extra security
